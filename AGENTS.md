...
- **Smoke-launch** — a green build does NOT prove the app starts: a
  ...
  **⚠ A dev build must never self-update, and it no longer can.** A dev build carries no
  `-Version` stamp, so its informational tag (`v1.0.14`) reads as older than the newest
  release; `StartupUpdateGate` used to apply the unattended self-update to it, downloading
  the SHIPPED `.exe` over `bin/Release/net8.0-windows/Aoe3ModLauncher.exe` and relaunching
  it — the run then reported on the released binary, not yours (measured: a 289 KB apphost
  ...
  build.` — **if neither line is there, no way out was true**, which is the oracle that found
...
- **"Run in background" is ONE toggle that bundles auto-start + minimise-to-tray
  ...
  **A hidden launch does less.** `RefreshNewsAsync` and the 5-minute `_catalogPollTimer`
  ...
  `App.StartMinimized`. `App.ShowMainWindow` then PARKS the window off-screen at its
  real size (`Services/TrayStartParking`, never `WindowState=Minimized` — a minimized
  window has a 0x0 frame and the `WindowChrome` that draws the whole title bar lays out
  against it, which is what made the window come back BLACK), calls `Show()` so it
  composes once, and hides it to the tray **after that `Show()` returns**
  (`MainWindow.HideToTrayAtStartup`). The self-update preserves the exe path, so the
  registered auto-start survives updates.
  **⚠ NOTHING MAY HIDE THE MAIN WINDOW FROM INSIDE `Window.Show()`, AND THE ONE TIME IT
  DID, THE LAUNCHER COULD NEVER BE OPENED AGAIN.** WPF's `Show()` creates the HWND and
  raises `Loaded` **before** it issues its own native `ShowWindow`, so the `HideToTray()`
  that used to live in the `Loaded` handler ran a nested show-helper and then let the
  outer `Show()` fire a now-stale `ShowWindow` at a window WPF believed was hidden.
  `TrayStartParking.ForeignShowDecision` read that as a show WPF had not asked for — its
  premise, *"WPF sets `Visibility` to Visible before its own `ShowWindow`"*, holds only
  while nobody hid the window in between — and answered with a raw `SW_HIDE`. From there
  WPF's `IsVisible` stayed `true` while the HWND had no `WS_VISIBLE`, so `Show()`
  early-returned without ever reaching `ShowWindow`, and since **every** route back (the
  tray icon, the tray menu, a deep link, a relaunch, a toast's button) funnels through
  `ShowFromTray`, the tray icon stopped opening the launcher **permanently**. A user's log
  has eleven clicks over three minutes, each answered with `visible=True … native=hidden`;
  `Get-Process` agreed, reporting `MainWindowHandle: 0` on a live process. Three things
  keep it shut: the hide happens after `Show()` returns (above); every `Show()` of the main
  window is wrapped in `TrayStartParking.EnterWpfShow`, which STATES that the show is WPF's
  own instead of making the guard infer it from `Visibility`; and `ShowFromTray` calls
  `TrayStartParking.ReconcileVisibility` afterwards, so a desync from any future cause is a
  logged blip that repairs itself rather than a launcher nobody can open. Pinned by
  `TrayStartParkingTests` — `THE_THIRD_REPORT_WpfsOwnShowIsNotForeignEvenWhenItsLoadedHandlerAlreadyHidIt`
  is the regression; note the pre-existing `WpfsOwnShowIsLeftAlone` passed throughout,
  because it fed the pure function the easy value (`Visibility.Visible`) that production
  never supplied. **The real check is launching the app** — no unit test can see a real
  `WS_VISIBLE`, and `MainWindowHandle != 0` is the cheapest honest oracle.
