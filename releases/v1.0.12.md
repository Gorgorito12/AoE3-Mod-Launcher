...
> **If you are on v1.0.9 or older, this is your update.** v1.0.10 was never published, and
> v1.0.11 shipped with a broken signature so the launcher correctly refused to install it —
> if you tried and got "Verification failed", that is why. Nothing was wrong with your PC.
> This release fixes it and includes everything both of those were meant to bring.
...
- **This now works for every mod, not just the ones that declared a save folder.** The launcher
  works out where a mod keeps its data on its own, so Improvement Mod and anything published in
  future get settings sharing, backups and better bug reports without their author doing anything.
...
- **An update that can't be installed now offers a manual download.** If verification fails or
  the download breaks, the dialog gives you a button to the release page instead of a dead end.
...
SHA256: A51A00825A8CD547BC3A97FF4B295F387F85B5040B02D75197073230E75DE4B4
