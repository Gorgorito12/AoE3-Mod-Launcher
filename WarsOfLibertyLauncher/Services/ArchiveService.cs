...
public class ArchiveService
{
    ...
    /// <summary>
    /// Extract a <c>.zip</c> archive into <paramref name="destinationFolder"/> with the SAME
    /// backup-before-overwrite + rollback contract as <see cref="ExtractTarXzWithBackupAsync"/>,
    /// used by the GitHubReleases incremental delta-patch path (the patch payload is a small zip,
    /// generated by the in-app patch tool). Returns the install-relative (forward-slash) paths
    /// this patch WROTE (created + overwritten); throws + rolls back on failure/cancel.
    /// </summary>
    public Task<IReadOnlyList<string>> ExtractZipWithBackupAsync(
        string archivePath,
        string destinationFolder,
        string backupFolder,
        IProgress<string>? progress = null,
        ...
        CancellationToken ct = default)
    {
        return Task.Run<IReadOnlyList<string>>(() =>
        {
            ...
            try
            {
                ...
                // Only real file entries (a directory entry has an empty Name).
                ...
                foreach (var entry in fileEntries)
                {
                    ...
                    // ZipArchiveEntry.FullName is forward-slash relative. Clamp to the
                    // destination root so a crafted zip can't escape via "..\" segments.
                    ...
                    {
                        DiagnosticLog.Write($"Delta patch: rejecting entry '{entry.FullName}' escaping the install root.");
                        continue;
                    }
                    ...
                    if (destExisted)
                    {
                        ...
                        if (!string.IsNullOrEmpty(backupDir))
                            Directory.CreateDirectory(backupDir);
                        File.Copy(destFile, backupFile, overwrite: true);
                        backedUp.Add(entry.FullName);
                    }

                    progress?.Report(WarsOfLibertyLauncher.Localization.Strings.Format("StatusExtracting", entry.FullName));
                    entry.ExtractToFile(destFile, overwrite: true);

                    if (!destExisted)
                        created.Add(entry.FullName);
